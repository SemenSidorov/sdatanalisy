...
if($iblock_id==5)
{
	...
	$ob_clinic = CIBLockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID"=>$IBLOCK_ID_CLINIC, "ID"=>$id, "ACTIVE"=>"Y"), array("ID", "NAME", "IBLOCK_ID", "CODE", "DETAIL_PAGE_URL", "IBLOCK_SECTION_ID", "IBLOCK_TYPE_ID", "PROPERTY_5"), false);
while($clinic = $ob_clinic->Fetch())
{
$ob_doctor = CIBLockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID"=>$IBLOCK_ID_DOCTOR, "ID"=>$clinic["PROPERTY_5_VALUE"], "ACTIVE"=>"Y"), array("ID", "NAME", "IBLOCK_ID", "CODE", "DETAIL_PAGE_URL", "IBLOCK_SECTION_ID", "IBLOCK_TYPE_ID"), false);
...
// выбираем все теги option с атрибутом value(ид привязанного элемента)
 var selected_mass = document.querySelectorAll('#tr_PROPERTY_5 > .adm-detail-content-cell-r > table > tbody > tr > td > table > tbody >  tr > td > select > option[selected]');
 //выбираем выше сформированный PHP массив
 var massiv =<?=CUtil::PhpToJSObject($ar_clinic)?>;
...
last = 'tr_PROPERTY_5 > .adm-detail-content-cell-r > table > tbody > tr > td > table > tbody > tr:nth-child('+index+') > td';
...
tag_a.setAttribute('name','tr_PROPERTY_PRIV_CLINIC_DOCTOR_'+index+'');
