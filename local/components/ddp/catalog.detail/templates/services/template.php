...
$show_404 = true;
$arFilter = Array("IBLOCK_ID"=>3, "ACTIVE"=>"Y", "CODE"=>$last_el);
...
$ob_city = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => 1, "ACTIVE"=>"Y", "ID" => $city_id), false, array("ID", "NAME", "CODE"));
$city = $ob_city -> Fetch();
$item = $arResult["ITEM"];
?>
<?
...
<?
	...
	$clinics = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => 5, "ACTIVE"=>"Y", "PROPERTY_CITY" => $city["ID"]), false, array());
	$arClinics = [];
	$arIdClinics = [];
	while($clinic = $clinics->GetNext()){
		$services = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => 9, "ACTIVE"=>"Y", "PROPERTY_SERVICES" => $item["ID"], "PROPERTY_CLINIC" => $clinic["ID"]), false, array());
		$services = $services->GetNext();
		$an_prop = CIBlockElement::GetProperty($services["IBLOCK_ID"], $services["ID"], array("SORT"=>"ASC"), array("CODE" => "PRICE"));
...
$an_prop_event = CIBlockElement::GetProperty($services["IBLOCK_ID"], $services["ID"], array("SORT"=>"ASC"), array("CODE" => "EVENT"));
		...
		$an_prop_event_price = CIBlockElement::GetProperty($services["IBLOCK_ID"], $services["ID"], array("SORT"=>"ASC"), array("CODE" => "EVENT_PRICE"));
...
<div class="services__description">
	<h2 class="middle__h2">Где можно получить услугу в регионе <?=$city["NAME"]?></h2>
	<div class="contacts-list">
		...
		<div class="contacts-list__item">
			<div class="contacts-list__item-contact">
				...
				<a class="contacts-list__item-phone" href="<?=$clinic["PHONE"]?>"><?=$clinic["PHONE"]?></a>
			</div>
			...
			<?if($clinic["EVENT"]==5 && !empty($clinic["PRICE"])):?>
			...
			<?elseif($clinic["EVENT"]==5 && empty($clinic["PRICE"])):?>
			<div class="contacts-list__item-price price"> <span class="price__num">от <?=$clinic["EVENT_PRICE"]?> ₽</span> <span class="price__rub"></span></div>
			<?elseif($clinic["EVENT"]!=5 && !empty($clinic["PRICE"])):?>
...
<?
$ob_norma = CIBlockElement::GetProperty(3, $item["ID"], array("SORT"=>"ASC"), array("CODE" => "NORMA"));
$norma = $ob_norma->Fetch();
$ob_examination = CIBlockElement::GetProperty(3, $item["ID"], array("SORT"=>"ASC"), array("CODE" => "EXAMINATION"));
$examination = $ob_examination->Fetch();
$ob_interpretation_result = CIBlockElement::GetProperty(3, $item["ID"], array("SORT"=>"ASC"), array("CODE" => "INTERPRETATION_RESULT"));
...
foreach($arClinics as $clinic){
$services_cost = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => 9, "ACTIVE"=>"Y", "PROPERTY_CLINIC" => $clinic["ID"]), false,  array("ID", "NAME", "CODE", "PROPERTY_SERVICES", "PROPERTY_PRICE", "PROPERTY_*"));
while($ob_services_cost = $services_cost->Fetch())
{
	...
	$cost_prop[] = $ob_services_cost;
		...
		{
			$services = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => 3, "ACTIVE"=>"Y", "ID" => $value["PROPERTY_SERVICES_VALUE"]), false, array("ID", "NAME", "CODE", "IBLOCK_SECTION_ID", "DETAIL_TEXT", "DETAIL_PAGE_URL"));
...
while($ob_analysis_cost = $analysis_cost->Fetch())
{
	...
	$cost_prop[] = $ob_analysis_cost;
		...
		foreach($cost_prop as $value)
		{
			$analyzes = CIBlockElement::GetList(array("SORT"=>"ASC"), array("IBLOCK_ID" => 2, "ACTIVE"=>"Y", "ID" => $value["PROPERTY_ANALYSIS_VALUE"]), false, false, array("ID", "NAME", "CODE", "IBLOCK_SECTION_ID", "DETAIL_TEXT", "DETAIL_PAGE_URL"));
		...
		<div class="analyzes-slider__item">
			...
			<div class="analyzes-slider__footer">
				<div class="analyzes-slider__price price">от <span class="price__num"><?=$event_price["VALUE"]?></span> <span class="price__rub">₽</span><span class="price__old-price">от <?=$final["VALUE"]?> ₽ </span></div>
	...
	<div class="analyzes-slider js-analyzes-slider" id="analyzes-slider js-analyzes-slider">
		...
		{
			$ob_section = CIBlockSection::GetList(array('SORT'=>'ASC'), array('IBLOCK_ID'=> 3, "ACTIVE"=>"Y", 'ID'=> $service["IBLOCK_SECTION_ID"], false, array("ID", "NAME", "CODE"), false));
			$section = $ob_section-> Fetch();
			$fin = CIBlockElement::GetProperty(9, $service["COST_ID"], "sort", "asc", array("CODE" => "PRICE"));
                          ...
                          $event_price = $event_price->Fetch();
						  $event_description = CIBlockElement::GetProperty(9, $service["COST_ID"], "sort", "asc", array("CODE" => "EVENT_DESCRIPTION"));
                          $event_description = $event_description->Fetch();
			?>
		<div class="analyzes-slider__item">
			...
			<div class="analyzes-slider__footer">
				<div class="analyzes-slider__price price">от <span class="price__num"><?=$event_price["VALUE"]?></span> <span class="price__rub">₽</span><span class="price__old-price">от <?=$final["VALUE"]?> ₽ </span></div>
				<a href="<?=$service["DETAIL_PAGE_URL"]?>" class="analyzes-slider__more btn btn--invert">Подробнее</a>
			</div>
		</div>
		<?}
		}
