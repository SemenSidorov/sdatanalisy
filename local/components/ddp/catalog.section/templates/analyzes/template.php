...
$rsSection = CIBlockSection::GetList(array(), array("IBLOCK_ID" => 2, "NAME"=>$arResult["title"]), false, false, array());
...
{	
$rsElement = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 2, "IBLOCK_SECTION_ID" => $arSection["ID"], "PROPERTY_CITIES"=>$city), false, array());
...
foreach ($Element_prop as $item)
	...
	$item["DETAIL_PAGE_URL"] = "/analyzes/".$arSection["CODE"]."/".$item["CODE"]."/";
	
	$clinics = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 5, "PROPERTY_CITY" => $city), false, array());
	
	$clinic = $clinics->GetNext();

	
	$analyzes = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 8, "PROPERTY_37" => $item["ID"], "PROPERTY_38" => $clinic["ID"]), false, array());
	$price = 0;
	$event_price = 0;
	$event = 0;
	while($ar_analyzes = $analyzes->GetNext()){
		//
		$an_event = CIBlockElement::GetProperty($ar_analyzes["IBLOCK_ID"], $ar_analyzes["ID"], array(), array("CODE" => "EVENT"));
		$an_event = $an_event->GetNext();
		$an_event_price = CIBlockElement::GetProperty($ar_analyzes["IBLOCK_ID"], $ar_analyzes["ID"], array(), array("CODE" => "EVENT_PRICE"));
		$an_event_price = $an_event_price->GetNext();
		$an_prop = CIBlockElement::GetProperty($ar_analyzes["IBLOCK_ID"], $ar_analyzes["ID"], array(), array("CODE" => "PRICE"));
